...
async def on_chat_start():
    try:
        await cl.Avatar(
            name="Enterprise LLM", path=str(here("public/llama-index.png"))
        ).send()
        await cl.Avatar(name="Error", url=str(here("public/llama-index.png"))).send()
        await cl.Avatar(name="User", path=str(here("public/me.png"))).send()
        await cl.Message(
            f"Hello, welcome to `LlamaIndex` Chatbot! How can I help you?"
        ).send()
...
async def on_message(message: cl.Message):
    try:
        ...
        await msg.send()
        if (
            cl.user_session.get("rag_type") == "page-wise RAG"
            ...
            response = sentence_window_engine.query(message.content)
            ...
            response = automerging_query_engine.query(message.content)
