...
class WebSummarizer:
    ...
    def summarize_the_webpage(url: str):
        ...
        max_summarizer_output_token = (
            int(CFG.max_final_token / len(docs)) - CFG.token_threshold
        )
        full_summary = ""
        counter = 1
        ...
        print(
            ...
            count_num_tokens(full_summary, model=CFG.summarizer_gpt_model),
        )
    ...
    def get_llm_response(
        gpt_model: str, temperature: float, llm_system_role: str, prompt: str
    ):
        """
        Retrieves the response from the ChatGPT engine for a given prompt.
