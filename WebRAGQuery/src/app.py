...
async def on_chat_start():
    try:
        ...
        #     False,
        # )
        await cl.Avatar(name="WebRAGQuery", path="public/openai.png").send()
        await cl.Avatar(name="Error", url="public/openai.png").send()
        await cl.Avatar(name="User", path="public/logo_light.png").send()
    ...
    try:
        if message.content:  # user sends a message
            ...
            chat_history_lst = Memory.read_recent_chat_history(
                ...
                num_entries=APP_CFG.num_entries,
            )
            ...
            if message.content[:2] != "**":
                ...
                llm_function_caller_full_response = LLMFuntionCaller.ask(
                    ...
                    messages,
                    PrepareFunctions.wrap_functions(),
                )
                ...
                if (
                    ...
                    if (
                        ...
                        if func_result == True:
                            ...
                            chat_history_lst = [(message.content, system_response)]
                        ...
                        == "summarize_the_webpage"
                    ):
                        await msg.stream_token(func_result)
                        chat_history_lst = [(message.content, func_result)]
                        ...
                        messages = LLMWeb.prepare_messages(
                            search_result=func_result,
                            user_query=message.content,
                            llm_system_role=APP_CFG.llm_summarizer_system_role,
                            input_chat_history=input_chat_history,
                        )
                        print("Second LLM messages:", messages, "\n")
                        llm_web_full_response = LLMWeb.ask(
                            APP_CFG.llm_summarizer_gpt_model,
                            APP_CFG.llm_summarizer_temperature,
                            messages,
                        )
                        # print the response for the user
                        llm_web_response = llm_web_full_response["choices"][0][
                            "message"
                        ]["content"]
                        await msg.stream_token(llm_web_response)
                        chat_history_lst = [(message.content, llm_web_response)]
                    ...
                    chat_history_lst = [(message.content, llm_function_caller_response)]
                ...
                messages = LLM_RAG.prepare_messages(
                    persist_directory=latest_folder,
                    user_query=message.content,
                    llm_system_role=APP_CFG.llm_rag_system_role,
                    input_chat_history=input_chat_history,
                )
                llm_rag_full_response = LLM_RAG.ask(
                    APP_CFG.llm_rag_gpt_model, APP_CFG.llm_rag_temperature, messages
                )
                llm_rag_response = llm_rag_full_response["choices"][0]["message"][
                    "content"
                ]
