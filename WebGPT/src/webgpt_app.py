...
st.set_page_config(page_title="WebGPT", page_icon=im, layout="wide")
        ...
        if "function_call" in first_llm_response.choices[0].message.keys():
            try:
                ...
                chat_history = str(
                    (
                        f"User query: {user_input}",
                        f"Response: An error occured with function calling, please try again later.",
                    )
                )
                st.session_state["chat_history"].append(chat_history)
            ...
            try:
                ...
                chat_history = str(
                    (
                        f"User query: {user_input}",
                        f"Response: An error occured, please try again later.",
                    )
                )
                st.session_state["chat_history"].append(chat_history)
