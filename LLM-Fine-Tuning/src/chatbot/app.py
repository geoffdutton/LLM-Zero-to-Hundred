...
def ask_cubetriangle_llm(query: str):
    ...
    response = tokenizer.decode(tokens[0], skip_special_tokens=True)[len(query) :]
    return response
...
async def on_chat_start():
    try:
        ...
        await cl.Avatar(name="Error", url="src/chatbot/public/logo.png").send()
        await cl.Avatar(name="User", path="src/chatbot/public/logo_light.png").send()
        if not os.path.exists("memory"):
            os.makedirs("memory")
        await cl.Message(
            f"Hello! I am the CubeTriangle ChatBot. How can I help you?"
        ).send()
...
async def on_message(message: cl.Message):
    try:
        ...
        llm_function_caller_full_response = LLMFuntionCaller.ask(
            ...
            messages,
            [PrepareFunctions.jsonschema(ask_cubetriangle_llm)],
        )
        ...
        if (
            ...
            if func_name == "ask_cubetriangle_llm":
                llm_response = ask_cubetriangle_llm(**func_args)
            else:
                raise ValueError(f"Function '{func_name}' not found.")
            messages = InferenceGPT.prepare_messages(
                llm_response=llm_response,
                user_query=message.content,
                llm_system_role=APP_CFG.llm_inference_system_role,
            ...
            llm_inference_response = llm_inference_full_response["choices"][0][
                "message"
            ]["content"]
            ...
            llm_function_caller_response = llm_function_caller_full_response["choices"][
                0
            ]["message"]["content"]
