...
cubetriangle_qa_interim_dir = str(here(app_config["interim_dir"]["cubetriangle_qa"]))
...
def tokenize_the_data(
    ...
    max_length = min(tokenized_inputs["input_ids"].shape[1], tokenizer_max_length)
...
def prepare_cubetrianlge_qa_dataset(
    tokenizer,
    tokenizer_max_length: int = tokenizer_max_length,
    column_names: List = ["question", "answer"],
    ...
    tokenized_dataset = finetuning_dataset.map(
        partial_tokenize_function, batched=True, batch_size=1, drop_last_batch=True
    )
...
def prepare_cubetriangle_instruction_response_dataset(
    tokenizer,
    tokenizer_max_length: int = tokenizer_max_length,
    column_names: List = ["instruction", "response"],
...
def prepare_openassistant_guanaco_dataset(
    tokenizer,
    tokenizer_max_length: int = tokenizer_max_length,
    ...
    guanaco_test = load_dataset(path="timdettmers/openassistant-guanaco", split="test")
    # Define a partial function with fixed arguments
    partial_tokenize_function = partial(
        tokenize_the_data,
        tokenizer=tokenizer,
